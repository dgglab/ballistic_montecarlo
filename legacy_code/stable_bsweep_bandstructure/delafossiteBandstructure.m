classdef delafossiteBandstructure < handle
    ...
    methods
        ...
        function init(obj)
            ...
            kf=obj.k0+obj.k6*cos(6.*theta_loop)+obj.k12*cos(12.*theta_loop);
            ...
            obj.K = obj.rotate(obj.K,obj.phi); % Rotate xtal axis
        ...
        function n = inject(obj,edgenorm)
            ...
            % If we are injecting from a single edge
            if length(edgenorm) == 1
                edge = find(edgenorm == obj.unorms);
                while isempty(n)
                    ind = randi([1,size(obj.inprob,2)]); % generate random index of Fermi surface
                    indf = ind + rand; % add some fraction to index
                    r = rand;
                    
                    inprobind = obj.inprob(edge,ind); % Probability of injecting at ind
                    if r < inprobind
                        n = indf;
                    end
                end
                ...
                % If we have been rounded to a corner, we
                % now consider two edge norms
            else
                ...
                while isempty(n)
                    ...
                    indf = ind + rand;
                    r = rand;
                    % Check if this index is allowed for edge1 and edge2
                    if obj.inprob(edge1,ind) ~= 0 && obj.inprob(edge2,ind) ~= 0
                        % Interpolate prob of injecting at indf
                        inprobind = obj.inprob(edge1,ind);
                        if r < inprobind
                            n = indf;
                        end
                    end
        ...
        %% Conductance for an aperature
        % w0: width of constriction in nm
        % theta: normal of constriction
        function [G, vTheta] = cond(obj, w0, theta)
            
            g = 4;
            e = 1.602E-19;
            h = 6.626E-34;
            
            dKx = obj.K(1,2:end)-obj.K(1,1:end-1);
            dKy = obj.K(2,2:end)-obj.K(2,1:end-1);
            dK = sqrt(dKx.^2 + dKy.^2);
            vTheta = wrapToPi(atan2(dKy,dKx)-pi/2);
            G = 0;
            c = 0;
            for i = 1:length(vTheta)
               if  wrapToPi(vTheta(i)-theta) < pi/2 && wrapToPi(vTheta(i)-theta) > -pi/2
                   G = G + (10*g*e^2*w0/(2*pi*h)) * cos(vTheta(i)-theta) * dK(i);
                   c = c + 1;
               end
            end
