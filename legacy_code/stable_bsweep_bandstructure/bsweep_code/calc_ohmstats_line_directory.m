...
for iDir = 1
    ...
    load([savepath,curDur,'\frameData.mat']);
    
    frm1=frmgrp.frms{1};
    
    addpath([savepath,curDur])
    filelist=ls([savepath,'masterclass','\*classes.mat']);
    
    linelist = ls([savepath,curDur,'/*lineohmstatsFIX.mat']);
    ...
    for iFile=1:Nfiles
        ...
        load([savepath,'masterclass','/',squeeze(filelist(iFile,:))]);
        
        edgenums(iFile,:)=edgenum;
        
        try
            ohmStats = load([savepath,curDur,'/',squeeze(linelist(iFile,:))]);
            edgenumsLine(iFile) = ohmStats.edgenumLine;
        catch
            edgenumsLine(iFile) = N_inject;
        end
    ...
    edgenumsLineAll(iDir,:) = edgenumsLine;
    edgenumsLineStore = edgenumsLineStore + edgenumsLine;
    N_injectStore = N_injectStore + N_inject;
    
    
    %% If storing rng seeds
    if rngseedstore
       load([savepath,curDur,'/rngData.mat'])
       rngseeds(iDir) = seed;
    end
...
edgenumsLine = edgenumsLineStore;
...
ohmNumsLine=edgenumsLine/N_inject;
ohmFluxLine=ohmNumsLine/frm1.ogs{1}.L;
...
% % for iFile = 1:8
% %     figure(1)
% %     plot(B_exp,edgenumstot(:,iFile))
% %     figure(2)
% %     plot(B_exp,N_inject*edgenumstot(:,iFile)./edgenumstot(:,8))
% % end
% figure(3); clf;
% plot(B_exp,var.^(1/2))
% xlabel('B (T)')
